...
void verticalorder(AVLNode *root, int ar[], int mid) {
    if(root) {
        ar[mid] += root->data;
//        cout << mid << ". "<<  ar[mid] << " " << root->data << endl; 
        verticalorder(root->left, ar, mid-1);
        verticalorder(root->right, ar, mid+1);
    }
}
...
void tebakkode(AVL *avl, int max) {
    ...
    verticalorder(avl->_root, ar, max/2);
    ll sum = 0 ; 
    for(i = 0; i < max+1; i++){
        if(ar[i] != 0){
        	sum += ar[i] * ar[i]; 
